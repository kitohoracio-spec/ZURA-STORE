...
function AuthPage({ goBack, showToast, goPage }) {
  ...
  async function handleForgotPassword() {
    setErro("");
    if(!email) { setErro("Insere o teu email para recuperar a senha."); return; }
    setLoading(true);
    try {
      await sendPasswordResetEmail(auth, email);
      showToast("Email de recuperação enviado! Verifica a tua caixa de entrada 📩");
    } catch(e) {
      setErro(e.code==="auth/user-not-found"?"Não existe conta com este email.":"Erro ao enviar email. Verifica o endereço.");
    } finally { setLoading(false); }
  }
...
function ContaPage({ user, goBack, showToast, goPage }) {
  ...
  const STATUS_COLOR = { pendente:"#c8a96e", processando:"#4e7cc8", enviado:"#4caf7d", entregue:"#4caf7d" };
          ...
          {loading ? (
            ...
            <div key={p.id} style={{ background:"#1a1a1a", border:"1px solid #2a2a2a", borderRadius:4, padding:20, marginBottom:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                ...
                <span style={{ background:STATUS_COLOR[p.status]||"#888", color:"#000", padding:"3px 10px", borderRadius:2, fontSize:"0.68rem", fontWeight:700, textTransform:"uppercase" }}>{p.status||"pendente"}</span>
              </div>
              <div style={{ fontSize:"0.88rem", marginBottom:8 }}>{p.produto}</div>
...
// ─── PRODUCT PAGE ─────────────────────────────────────────────────────────────
function ProductPage({ product:p, qty, setQty, wishlist, toggleWish, addToCart, goBack, openProduct, showToast, user, goPage }) {
...
const CARREIRAS=[{title:"Trabalha connosco",text:"A ZURA procura pessoas apaixonadas por moda e cultura moçambicana."},{title:"Vagas Actuais",items:["Designer de Moda — Nampula","Gestor de Redes Sociais — Remoto","Responsável de Logística — Nampula","Atendimento ao Cliente — Nampula"]},{title:"Como Candidatar",text:"Envia CV para carreiras@zurastore.co.mz com o título da vaga."},{title:"Estágios",text:"Recebemos estagiários em design, marketing e gestão. Mínimo 3 meses."}];
