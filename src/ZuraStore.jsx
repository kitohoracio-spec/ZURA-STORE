...
function AdminPage({ user, isStaff, isSuperAdmin, userRole, goBack, showToast, products, refreshProducts }) {
  ...
  useEffect(()=>{
    if(!isStaff) return;
    fetchPedidos();
    if(isSuperAdmin){ fetchMensagens(); fetchUsuarios(); }
  },[isStaff]);
  ...
  const TABS = isSuperAdmin
    ? [["pedidos","📦 Pedidos"],["produtos","🏷️ Produtos"],["mensagens","✉️ Mensagens"],["usuarios","👥 Utilizadores"],["seed","⚡ Configuração"]]
      ...
      {tab==="mensagens" && isSuperAdmin && (
        <div>
          {loadingMsgs ? <p style={{color:"#888"}}>A carregar mensagens...</p> :
           ...
           mensagens.map(m=>(
            <div key={m.id} style={{ background:"#1a1a1a", border:`1px solid ${m.lida?"#2a2a2a":A}`, borderRadius:4, padding:18, marginBottom:14 }}>
              ...
              <p style={{ color:"#ccc", fontSize:"0.88rem", lineHeight:1.7, marginBottom:m.lida?0:12 }}>{m.mensagem}</p>
              {!m.lida && <button onClick={()=>marcarLida(m.id)} style={{ background:"transparent", border:"1px solid #4caf7d", color:"#4caf7d", padding:"6px 14px", borderRadius:2, cursor:"pointer", fontSize:"0.75rem" }}>Marcar como lida</button>}
            </div>
          ))}
        </div>
      )}
  ...
  async function enviar(){
    ...
    try {
      await addDoc(collection(db,"mensagens"), {
        ...
        lida: false, criadoEm: serverTimestamp(),
      });
