...
export default function ZuraStore() {
  ...
  return (
    <div style={ROOT}>
      ...
      {page==="contacto"   && <ContactoPage goBack={()=>goPage("home")} showToast={showToast} user={user} />}
...
function ProductEditor({ produto, onSave, onCancel }) {
  ...
  async function handleImageUpload(e){
    ...
    setUploading(true);
    try {
      const path = `produtos/${form.id}_${Date.now()}_${file.name}`;
      ...
      console.error(e);
      setUploadError("Erro ao enviar imagem: "+(e.code||e.message));
    } finally { setUploading(false); }
  }

  async function removeImage(){
    if(form.imagePath){
      try { await deleteObject(ref(storage, form.imagePath)); } catch(e){ console.log(e); }
    }
  ...
  function handleSave(){
    ...
    if(!cleaned.imageUrl){ delete cleaned.imageUrl; delete cleaned.imagePath; }
...
// ─── CONTACTO ─────────────────────────────────────────────────────────────────
function ContactoPage({ goBack, showToast, user }) {
  const [form,setForm]=useState({nome:user?.displayName||"",email:user?.email||"",tel:"",msg:""});
  const [sending,setSending]=useState(false);
  ...
  async function enviar(){
    ...
    try {
      await addDoc(collection(db,"mensagens"), {
        ...
        nome: form.nome, email: form.email, telefone: form.tel, mensagem: form.msg,
        lida: false, respondida: false, resposta: null, criadoEm: serverTimestamp(),
      });
      showToast("Mensagem enviada! Responderemos em breve 📩");
      setEnviada(true);
      setForm({nome:user?.displayName||"",email:user?.email||"",tel:"",msg:""});
    } catch(e){
      console.error(e);
    ...
    <PageWrap>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:60 }}>
        <div>
          ...
          {user && (
            <div style={{ background:"rgba(200,169,110,0.06)", border:"1px solid rgba(200,169,110,0.2)", borderRadius:4, padding:"12px 16px", fontSize:"0.8rem", color:"#aaa" }}>
              💡 Como tens conta, vais poder ver a nossa resposta na aba <strong style={{color:A}}>"Mensagens"</strong> da tua conta.
            </div>
          )}
        </div>
        <div>
          {enviada ? (
            <div style={{ background:"rgba(76,175,125,0.08)", border:"1px solid rgba(76,175,125,0.3)", borderRadius:4, padding:28, textAlign:"center" }}>
              <div style={{ fontSize:"2.5rem", marginBottom:14 }}>✅</div>
              <h3 style={{ fontFamily:"Georgia,serif", fontSize:"1.3rem", marginBottom:10, color:"#7ed9af" }}>Mensagem Enviada!</h3>
              <p style={{ color:"#aaa", fontSize:"0.88rem", lineHeight:1.8, marginBottom:18 }}>Recebemos a tua mensagem e vamos responder em breve.</p>
              <GhostBtn onClick={()=>setEnviada(false)}>Enviar outra mensagem</GhostBtn>
            </div>
          ) : (
            <>
              ...
              <Btn disabled={sending} onClick={enviar} style={{ width:"100%",padding:14 }}>{sending?"A enviar...":"Enviar Mensagem"}</Btn>
            </>
          )}
        </div>
      </div>
