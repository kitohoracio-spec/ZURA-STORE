...
export default function ZuraStore() {
  ...
  return (
    <div style={{...ROOT, paddingBottom:64}}>
      {/* TOP NAV — logo + categorias, esconde ao scroll para baixo */}
      <nav style={{ position:"sticky", top:0, zIndex:100, background:"var(--bg-root)", opacity:0.97, backdropFilter:"blur(12px)", borderBottom:"1px solid var(--border)", padding:"0 4%", display:"flex", alignItems:"center", justifyContent:"space-between", height:64, gap:16, transform:navVisible?"translateY(0)":"translateY(-100%)", transition:"transform 0.25s ease" }}>
        ...
        <ul style={{ display:"flex", gap:22, listStyle:"none", margin:0, padding:0, overflowX:"auto" }}>
          {["Novidades","Mulher","Homem","Acessórios","Vendidos"].map(l=>(
            <li key={l}><NavLink label={l} onClick={()=>navFilter(l)}/></li>
          ))}
        </ul>
        {searchOpen && (
          <div style={{ display:"flex", alignItems:"center", gap:6, background:"var(--bg-input)", border:`1px solid ${A}`, borderRadius:2, padding:"4px 8px", flexShrink:0 }}>
            <input autoFocus type="text" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}
              ...
              placeholder="Procurar produtos..." style={{ background:"none", border:"none", color:"var(--text)", fontSize:"0.82rem", outline:"none", width:140 }}/>
      ...
      {/* BOTTOM NAV — estilo Pinterest/Instagram, ícones outline sem cor */}
      <nav style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:100, background:"var(--bg-root)", borderTop:"1px solid var(--border)", display:"flex", alignItems:"center", justifyContent:"space-around", height:60, padding:"0 4px" }}>
        ...
        <BottomNavIcon active={page==="conta"||page==="auth"||page==="admin"} onClick={()=>{ setContaTabInicial("pedidos"); goPage(user?"conta":"auth"); }} label="Conta" badge={pedidosNaoVistos>0?pedidosNaoVistos:null}
          icon={<><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 4-6 8-6s8 2 8 6"/></>}/>
      </nav>

      <div style={{display:"none"}}>{/* old nav cart/account icons moved to bottom bar */}</div>
...
function MensagemCard({ msg:m, marcarLida, responderMensagem, apagarMensagemAdmin }) {
  ...
  return (
    <div style={{ background:"var(--bg-card)", border:`1px solid ${m.lida?"var(--border)":A}`, borderRadius:4, padding:18, marginBottom:14 }}>
      ...
      <p style={{ color:"var(--text-dim3)", fontSize:"0.88rem", lineHeight:1.7, marginBottom:14 }}>{m.mensagem}</p>
