...
function NavLink({ label, onClick }) {
  ...
  return (
    <span onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ position:"relative", display:"inline-block", color:hov?"var(--text)":"var(--text-dim)", fontSize:"0.72rem", letterSpacing:0.5, textTransform:"uppercase", cursor:"pointer", fontWeight:400, transition:"color 0.2s", padding:"4px 0", whiteSpace:"nowrap" }}>
      {label}
      <span style={{ position:"absolute", left:0, bottom:-2, height:2, width:hov?"100%":"0%", background:A, transition:"width 0.25s ease" }}/>
    </span>
  );
}
      ...
      <nav style={{ position:"sticky", top:0, zIndex:100, background:"var(--bg-root)", opacity:0.97, backdropFilter:"blur(12px)", borderBottom:"1px solid var(--border)", padding:"8px 4%", transform:navVisible?"translateY(0)":"translateY(-100%)", transition:"transform 0.25s ease" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, maxWidth:1100, margin:"0 auto" }}>
          ...
          {searchOpen ? (
            ...
            <ul style={{ display:"flex", gap:13, listStyle:"none", margin:0, padding:0, overflowX:"auto", justifyContent:"center", flex:1, scrollbarWidth:"none" }}>
              {["Novidades","Mulher","Homem","Acessórios","Vendidos"].map(l=>(
                <li key={l} style={{flexShrink:0}}><NavLink label={l} onClick={()=>navFilter(l)}/></li>
              ))}
            </ul>
          )}
          ...
          {loadingPedidos ? <p style={{color:"var(--text-dim)"}}>A carregar pedidos...</p> :
           ...
           pedidos.map(p=>(
            <div key={p.id} style={{ background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:4, padding:18, marginBottom:14 }}>
              ...
              <button onClick={()=>imprimirEtiqueta(p)} style={{ width:"100%", marginTop:12, background:"transparent", border:"1px solid var(--border2)", color:"var(--text-dim)", padding:"8px", borderRadius:2, cursor:"pointer", fontSize:"0.72rem", letterSpacing:1, textTransform:"uppercase", fontWeight:600 }}>
                🖨️ Imprimir Etiqueta
              </button>
            </div>
          ))}
        ...
        <div>
          ...
          {loading ? (
            ...
            <div key={p.id} style={{ background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:4, padding:20, marginBottom:16 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={()=>imprimirEtiqueta(p)} style={{ flex:1, background:"transparent", border:"1px solid var(--border2)", color:"var(--text-dim)", padding:"9px", borderRadius:2, cursor:"pointer", fontSize:"0.76rem", letterSpacing:1, textTransform:"uppercase", fontWeight:600 }}>
                  🖨️ Imprimir Etiqueta
                </button>
                {p.status==="pendente" && (
                  <button onClick={()=>cancelarPedido(p.id)} style={{ flex:1, background:"transparent", border:"1px solid #c84e4e", color:"#c84e4e", padding:"9px", borderRadius:2, cursor:"pointer", fontSize:"0.76rem", letterSpacing:1, textTransform:"uppercase", fontWeight:600 }}>
                    Cancelar Pedido
                  </button>
                )}
              </div>
      ...
      {activeTab==="mensagens" && (
        <div>
          ...
          {loadingMsgs ? (
            ...
            <div key={m.id} style={{ background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:4, padding:20, marginBottom:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12, gap:12 }}>
                <div style={{ fontSize:"0.78rem", color:"var(--text-dim4)" }}>{m.criadoEm?.toDate?.()?.toLocaleDateString("pt-MZ")||"—"}</div>
