...
function NavLink({ label, onClick }) {
  ...
  return (
    <span onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ position:"relative", display:"inline-block", color:hov?"var(--text)":"var(--text-dim)", fontSize:"0.8rem", letterSpacing:1, textTransform:"uppercase", cursor:"pointer", fontWeight:400, transition:"color 0.2s", padding:"4px 0" }}>
      {label}
...
export default function ZuraStore() {
  ...
  return (
    <div style={{...ROOT, paddingBottom:64}}>
      {/* TOP NAV — linha 1: logo + conta | linha 2: categorias centralizadas. Esconde ao scroll para baixo */}
      <nav style={{ position:"sticky", top:0, zIndex:100, background:"var(--bg-root)", opacity:0.97, backdropFilter:"blur(12px)", borderBottom:"1px solid var(--border)", padding:"10px 4% 12px", display:"flex", flexDirection:"column", gap:10, transform:navVisible?"translateY(0)":"translateY(-100%)", transition:"transform 0.25s ease" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <span onClick={()=>goPage("home")} style={{ display:"flex", alignItems:"center", cursor:"pointer" }}>
            <ZuraMonogram size={46}/>
          </span>

          {searchOpen ? (
            <div style={{ display:"flex", alignItems:"center", gap:6, background:"var(--bg-input)", border:`1px solid ${A}`, borderRadius:2, padding:"4px 8px", flexShrink:0 }}>
              <input autoFocus type="text" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}
                ...
                placeholder="Procurar produtos..." style={{ background:"none", border:"none", color:"var(--text)", fontSize:"0.82rem", outline:"none", width:120 }}/>
            ...
            <button onClick={()=>{ setContaTabInicial("pedidos"); goPage(user?"conta":"auth"); }} style={{ background:"none", border:"none", cursor:"pointer", padding:6, position:"relative", display:"flex" }}>
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke={(page==="conta"||page==="auth"||page==="admin")?A:"var(--text-dim)"} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
        ...
        <ul style={{ display:"flex", gap:20, listStyle:"none", margin:0, padding:0, overflowX:"auto", justifyContent:"center" }}>
      ...
      <nav style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:100, background:"var(--bg-root)", borderTop:"1px solid var(--border)", display:"flex", alignItems:"center", justifyContent:"space-around", height:60, padding:"0 4px", transform:navVisible?"translateY(0)":"translateY(100%)", transition:"transform 0.25s ease" }}>
