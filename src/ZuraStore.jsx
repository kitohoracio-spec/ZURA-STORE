...
function NavLink({ label, onClick, special=false }) {
  const [hov, setHov] = useState(false);
  return (
    <span onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ position:"relative", display:"inline-block", color:special?A:(hov?"var(--text)":"var(--text-dim)"), fontSize:"0.8rem", letterSpacing:1, textTransform:"uppercase", cursor:"pointer", fontWeight:special?700:400, transition:"color 0.2s", padding:"4px 0" }}>
      {special && <span style={{ position:"absolute", top:-9, right:-12, width:6, height:6, borderRadius:"50%", background:"#c84e4e" }}/>}
      {label}
      <span style={{ position:"absolute", left:0, bottom:-2, height:2, width:hov||special?"100%":"0%", background:A, transition:"width 0.25s ease" }}/>
    </span>
  );
}
...
export default function ZuraStore() {
  ...
  return (
    <div style={ROOT}>
      {/* NAV */}
      <nav style={{ position:"sticky", top:0, zIndex:100, background:"var(--bg-root)", opacity:0.97, backdropFilter:"blur(12px)", borderBottom:"1px solid var(--border)", padding:"0 4%", display:"flex", alignItems:"center", justifyContent:"space-between", height:64, gap:16 }}>
        ...
        <ul style={{ display:"flex", gap:22, listStyle:"none", margin:0, padding:0 }}>
          {["Novidades","Mulher","Homem","Acessórios","Vendidos"].map(l=>(
            <li key={l}><NavLink label={l} onClick={()=>navFilter(l)} special={l==="Vendidos"}/></li>
          ))}
        </ul>
        <div style={{ display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
          {searchOpen ? (
            <div style={{ display:"flex", alignItems:"center", gap:6, background:"var(--bg-input)", border:`1px solid ${A}`, borderRadius:2, padding:"4px 8px" }}>
            ...
            <span onClick={()=>setSearchOpen(true)} style={{ cursor:"pointer", fontSize:"1rem", color:"var(--text-dim)" }}>🔍</span>
          )}
          {user ? (
            <>
              <span onClick={()=>goPage("conta")} style={{ display:"flex", alignItems:"center", gap:8, color:A, fontSize:"0.8rem", cursor:"pointer", letterSpacing:1, position:"relative" }}>
                <span style={{ width:26, height:26, borderRadius:"50%", background:`linear-gradient(135deg,#2a1f0e,${A})`, color:"#000", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.72rem", fontWeight:700, flexShrink:0 }}>
              ...
              <button onClick={handleLogout} style={{ background:"transparent", border:"1px solid var(--border2)", color:"var(--text-dim)", padding:"6px 14px", borderRadius:2, fontSize:"0.75rem", cursor:"pointer", letterSpacing:1, textTransform:"uppercase", display:"flex", alignItems:"center", gap:6 }}>
                <span style={{fontSize:"0.85rem"}}>🚪</span> Sair
              </button>
            </>
          ) : (
            <button onClick={()=>goPage("auth")} style={{ background:"transparent", border:`1px solid ${A}`, color:A, padding:"6px 14px", borderRadius:2, fontSize:"0.78rem", cursor:"pointer", letterSpacing:1, textTransform:"uppercase", display:"flex", alignItems:"center", gap:6 }}>
              <span style={{fontSize:"0.85rem"}}>👤</span> Entrar
            </button>
          )}
...
function CartPanel({ cart, cartTotal, user, changeQty, removeFromCart, setCart, setCartOpen, showToast }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:200 }}>
      ...
      <div style={{ position:"absolute", right:0, top:0, bottom:0, width:380, maxWidth:"100vw", background:"#161616", borderLeft:"1px solid var(--border)", display:"flex", flexDirection:"column", zIndex:1 }}>
    ...
    <>
      ...
      <div style={{ margin:"0 5% 72px", background:"linear-gradient(135deg,#1e1508,#2d2010)", border:"1px solid rgba(200,169,110,0.2)", borderRadius:4, padding:"48px 56px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:24, position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"relative" }}>
          ...
          <p style={{ color:"var(--text-dim)", fontSize:"0.88rem" }}>Apenas até ao fim do mês.</p>
        ...
        <div style={{ textAlign:"right", position:"relative" }}>
          ...
          <div style={{ fontSize:"0.78rem", letterSpacing:2, textTransform:"uppercase", color:"var(--text-dim)" }}>de desconto</div>
        </div>
      </div>
